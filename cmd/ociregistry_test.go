...
func TestCmdline(t *testing.T) {
	...
	cmdline := "server --config-path %s --image-path %s --log-level %s --port %s --load-images %s" +
		" --preload-images %s --arch %s --os %s --list-cache --prune %s --dry-run --version --always-pull-latest --concurrent %d --pull-timeout %d"
	cmdline = fmt.Sprintf(cmdline, cfgPath, imgPath, logLvl, port, loadImages, preloadImages, arch, opsys, prune, concurrent, pullTimeout)
