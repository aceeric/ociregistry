...
func TestNoAuthHttp(t *testing.T) {
	...
	_, err := cranePull("localhost:5000/hello-world:latest")
	if err != nil {
		t.Errorf(err.Error())
	}
}
...
func TestBasicAuthHttp(t *testing.T) {
	...
	parseConfig([]byte(sl(cfg)))
	os.Remove("/tmp/deleteme.tar")
	_, err := cranePull("localhost:5001/hello-world:latest")
	if err != nil {
		t.Errorf(err.Error())
	}
}
...
func TestNoAuthHttps1WayNoTlsConfig(t *testing.T) {
	...
	parseConfig([]byte(sl(cfg)))
	_, err := cranePull("localhost:8443/hello-world:latest")
	if err == nil {
...
func TestNoAuthHttps1WaySecure(t *testing.T) {
    ...
    - name: localhost:8443
      ...
      tls:
        ca: /home/eace/projects/ociregistry/test-registry-servers/nginx/certs/ca.crt`
...
func TestNoAuthHttps2Way(t *testing.T) {
    ...
    - name: localhost:8444
      ...
      tls:
        ca:   /home/eace/projects/ociregistry/test-registry-servers/nginx/certs/ca.crt
        cert: /home/eace/projects/ociregistry/test-registry-servers/nginx/certs/localhost.crt
        key:  /home/eace/projects/ociregistry/test-registry-servers/nginx/certs/localhost.key`

	parseConfig([]byte(sl(cfg)))
	os.Remove("/tmp/deleteme.tar")
	_, err := cranePull("localhost:8444/hello-world:latest")
	if err != nil {
		t.Errorf(err.Error())
	}
}
